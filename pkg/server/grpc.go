...
func (s *GrpcServer) Start(cancel context.CancelFunc) {
	...
	if err != nil {
		s.logger.Error("failed to load server certificate: " + err.Error())
		cancel()
		return
	}

	// Создаем gRPC credentials с TLS конфигурацией
	creds := credentials.NewTLS(sslConf)

	conn, errListen := net.Listen("tcp", ":"+s.cfg.Port)
	if errListen != nil {
		s.logger.Error("failed to listen on port " + s.cfg.Port + ": " + errListen.Error())
		cancel()
		return
	}

	s.server = grpc.NewServer(
		grpc.Creds(creds), // Добавляем TLS credentials
